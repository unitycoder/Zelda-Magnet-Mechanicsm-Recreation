...
namespace InputSystem
{
    public class InputManager : MonoBehaviour
    {
        ...
        public bool TryRegisterToInputListener<T>(EInputEvent inputEvent, Action<T> callback)
            where T : InputData
        {
            InputListenerBase listener = _InputListeners.SingleOrDefault(i 
                => i.GetInputEventType().Equals(inputEvent));
            ...
            Debug.LogWarning($"Input Listener could not found for {inputEvent}");
